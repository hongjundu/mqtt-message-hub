...
int httpdModifyVariable(request *r, const char *name, const char *value)
{
	httpVar *curVar, *lastVar, *newVar;
		...
		if (inHeaders)
		{
			...
			*/
            ...
            if (strncasecmp(buf,"User-Agent: ",12) == 0)
            {
                ...
                {
                    cp += 2;
                    strncpy(r->request.userAgent, cp, HTTP_MAX_URL);
            ...
            if (strncasecmp(buf,"X-Forwarded-For: ",17) == 0)
            {
                ...
                {
                    cp += 2;
                    strncpy(r->request.ipForwardedFor, cp, HTTP_IP_ADDR_LEN);
                    r->request.ipForwardedFor[HTTP_IP_ADDR_LEN-1]=0;
                }
            }
...
#else
        r->request.body_len = r->request.contentLength < HTTP_MAX_URL - 1 ?
                              r->request.contentLength : HTTP_MAX_URL - 1;
        _httpd_readBuf(r, r->request.body, r->request.body_len);
        r->request.body[r->request.body_len] = 0;
#endif
